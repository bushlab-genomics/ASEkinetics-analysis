...
def main():
	maf = 0.2
	# input kinetics
	kinetics_input = sys.argv[1]	
	kinetics = np.loadtxt(kinetics_input)
	sample_bs, sample_bf, sample_r = kinetics

	log2fc_bs = float(sys.argv[2])
	log2fc_bf = float(sys.argv[3])
	...
	# comparable kinetics
	sample_bs2 = sample_bs * np.power(2, log2fc_bs)
	sample_bf2 = sample_bf * np.power(2, log2fc_bf)
	...
	kp_df = pd.DataFrame(columns=['kon', 'koff', 'ksyn', 'mean','var', 'bs','bf'])
	# estimate kineicts
	for i, row in expression_df.iterrows():
		...
		var  = np.var(row.values)

		record = [ kon, koff, ksyn, mean, var, bs, bf ]
		kp_df.loc[i,:] = record
	...
	res_df['log2fc_bs'] = log2fc_bs
	res_df['log2fc_bf'] = log2fc_bf
